...
module "cassandra-gcp-cluster-nodepool" {
  ...
  project_id         = var.project_id
  cluster_name       = module.cassandra-gcp-cluster.name
  location           = var.location
  name               = format("%s-nodepool", module.cassandra-gcp-cluster.name)
  initial_node_count = var.gke_initial_node_count
  node_machine_type  = var.gke_node_type
  node_preemptible   = true
  node_tags          = ["google-notr-cassandra"]
  autoscaling_config = {
    min_node_count = 3
    max_node_count = 5
  }
